...
func (e *EmailService) SendEmail(Sender string, Recipient string, Subject string, TextBody string, emailType string, payload interface{})(string, error) {

	t := template.New("emailContentTemplate")

	data, _ := ioutil.ReadFile("./template/" + emailType + ".html")

	emailTemplate := model.EmailTemplate{Recipient:Recipient, ResetPasswordURL:payload}
